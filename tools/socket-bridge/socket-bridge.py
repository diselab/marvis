    ...
    def do_PUT(self):
        if self.path.startswith('/socket/'):
            ...
            if switch_id > 15:
                ...
                return
            if switch_toggle == 'on':
                print('sending', ids[switch_id], 'XOR', on, 'ergo', ids[switch_id] ^ on)
                subprocess.run(["./transmit", str(ids[switch_id] ^ on), str(protocol)])
                self.send_response(200, "OK")
                self.end_headers()
            elif switch_toggle == 'off':
                print('sending', ids[switch_id], 'XOR', off, 'ergo', ids[switch_id] ^ off)
                subprocess.run(["./transmit", str(ids[switch_id] ^ off), str(protocol)])
                self.send_response(200, "OK")
                self.end_headers()
            else:
